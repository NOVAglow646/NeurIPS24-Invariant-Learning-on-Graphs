...
class CIGAabl(BaseOODAlg):
    r"""
    CIGA for ablation study
    """
    def __init__(self, config: Union[CommonArgs, Munch]):
        super(CIGAabl, self).__init__(config)
        self.rep_out = None
    ...
    def loss_calculate(self, raw_pred: Tensor, targets: Tensor, mask: Tensor, node_norm: Tensor,
        ...
        if self.rep_out is not None:
            ...
            contrastive_loss_weight = config.ood.extra_param[2]

            if self.step <= -1:
                loss = cls_loss
            else:
                loss = cls_loss + config.ood.extra_param[0] * contrastive_loss_weight * contrast_loss + \
                            (config.ood.extra_param[1] if len(config.ood.extra_param)>1 else 0) * \
                                (1-contrastive_loss_weight) * hinge_loss
            self.mean_loss = cls_loss
            
            self.spec_loss = contrastive_loss_weight *contrast_loss + (1-contrastive_loss_weight )* hinge_loss
        else:
