...
def eval_data_preprocess(y: torch.Tensor,
    ...
    if config.metric.dataset_task == 'Binary classification' and config.ood.ood_alg!='sp':
    ...
    elif config.metric.dataset_task == 'Multi-label classification' or config.ood.ood_alg=='sp':
        pred_prob = raw_pred.softmax(dim=1)
        
        pred = pred_prob[mask].detach().cpu().numpy()
        #print(f'#in# mask {mask}')
        #print(f'#in# in pred {pred}')
