...
class CIGAdiv2GIN(GNNBasic):

    def __init__(self, config: Union[CommonArgs, Munch]):
        super(CIGAdiv2GIN, self).__init__(config)
    ...
    def forward(self, *args, **kwargs):
        ...
        causal_rep = self.get_graph_rep(data=Data(x=causal_x,
        ...
        if self.training:
            ...
            spu_rep = self.get_graph_rep(data=Data(x=spu_x, edge_index=spu_edge_index, edge_attr=spu_edge_attr, batch=spu_batch),
                                         batch_size=batch_size)  #.detach()

            # reverse gradient
            spu_rep = GradientReverseLayerF.apply(spu_rep, self.config.train.alpha)
...
class GAEAttNet(nn.Module):
    ...
    def __init__(self, causal_ratio, config, **kwargs):
        ...
        self.hidden = config_catt.model.dim_hidden
        self.linear1 = nn.Linear(self.hidden * 2, self.hidden)
        self.linear2 = nn.Linear(self.hidden, self.hidden // 2)
        self.linear3 = nn.Linear(self.hidden // 2, 1)
        self.relu1 = nn.ReLU()
        self.relu2 = nn.ReLU()
    ...
    def forward(self, *args, **kwargs):
        ...
        mid_rep1 = self.relu1(self.linear1(edge_rep))
        mid_rep2=self.relu2(self.linear2(mid_rep1))
        edge_score=self.linear3(mid_rep2).view(-1)
