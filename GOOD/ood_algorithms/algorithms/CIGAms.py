...
class CIGAms(BaseOODAlg):
    ...
    def __init__(self, config: Union[CommonArgs, Munch]):
        super(CIGAms, self).__init__(config)
        self.rep_out =None
        self.causal_out =None
        self.spu_out_covariate = None
        self.spu_out_concept = None
        self.step=0
    ...
    def output_postprocess(self, model_output: Tensor, **kwargs) -> Tensor:
        ...
        if isinstance(model_output, tuple):
            self.rep_out,  self.causal_out, self.spu_out_covariate, self.spu_out_concept = model_output
        else:
    ...
    def loss_calculate(self, raw_pred: Tensor, targets: Tensor, mask: Tensor, node_norm: Tensor,
        ...
        if self.rep_out is not None:
            ...
            spu_loss_covariate = config.metric.loss_func(self.spu_out_covariate, targets, reduction='none')
            spu_loss_concept = config.metric.loss_func(self.spu_out_concept, targets, reduction='none')
            # print(causal_loss.sum(),spu_loss.sum())
            #print('#IN#spu_loss', spu_loss_concept.shape) #[32]
            ...
            if len(config.ood.extra_param)>1:
                # hinge loss
                spu_loss_weight= torch.zeros(spu_loss_concept.size()).to(raw_pred.device) # used for both cov and con sp_loss.
                spu_loss_weight[spu_loss_concept > causal_loss] = 1.0
                spu_loss_weight = spu_loss_weight * mask
                spu_loss_covariate = (spu_loss_covariate * spu_loss_weight).sum() / (spu_loss_weight.sum() + 1e-6)
                spu_loss_concept = (spu_loss_concept * spu_loss_weight).sum() / (spu_loss_weight.sum() + 1e-6)
                hinge_loss = (config.ood.extra_param[1] if len(config.ood.extra_param)>1 else 0) * spu_loss_concept\
                            - (config.ood.extra_param[1] if len(config.ood.extra_param)>1 else 0) * spu_loss_covariate
            else:
                hinge_loss = 0
            ...
            if self.step <= -1:
                ...
                loss = cls_loss + config.ood.extra_param[0] * contrast_loss + hinge_loss
