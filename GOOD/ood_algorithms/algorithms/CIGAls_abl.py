...
def smooth_ce_loss(pred_class_logits, gt_classes, gamma=0.1, alpha=0.2, reduction='none'):
...
class CIGAls_abl(BaseOODAlg):
    ...
    def __init__(self, config: Union[CommonArgs, Munch]):
        super(CIGAls_abl, self).__init__(config)
        self.rep_out = None
    ...
    def loss_calculate(self, raw_pred: Tensor, targets: Tensor, mask: Tensor, node_norm: Tensor,
        ...
        if self.rep_out is not None:
            ...
            spu_loss=smooth_ce_loss(self.spu_out,targets,config.ood.extra_param[2],alpha=0.2,reduction='none')
            #print("#IN#",spu_loss.shape)
