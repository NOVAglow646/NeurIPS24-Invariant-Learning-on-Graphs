...
class GOODCBAS(InMemoryDataset):
    ...
    def get_covariate_shift_graph(self, data_list, graph):
        ...
        num_data = self.num_data
        print(F'#in# self.num_data={self.num_data}')
        train_ratio = 0.8
        val_ratio = 0.1
        test_ratio = 0.1
        train_split = int(num_data * train_ratio)
    ...
    def load(dataset_root: str, domain: str, shift: str = 'no_shift', generate: bool = False):
        ...
        #print(F'#in# dataset[1] {dataset[1]}')
        #print(F'#in# len(dataset) {len(dataset)}')
        #print(F'#in# meta_info.num_train_nodes {meta_info.num_train_nodes}') #420
