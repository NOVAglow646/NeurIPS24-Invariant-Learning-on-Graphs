...
def smooth_ce_loss(pred_class_logits, gt_classes, eps=0.1, alpha=0.2, reduction='none'):
    num_classes = pred_class_logits.size(1)
    #print('#IN# pred_class_logits',pred_class_logits.shape)
    #print('#IN# gt_classes',gt_classes.shape)
    if eps >= 0:
        smooth_param = eps
    else:
    ...
    #print('#IN# loss',loss.shape)

    with torch.no_grad():
        non_zero_cnt = max(loss.nonzero(as_tuple=False).size(0), 1)
    #print('#IN#',non_zero_cnt)
    if reduction != 'none':
...
class CIGAdiv2ls(BaseOODAlg):
    ...
    def __init__(self, config: Union[CommonArgs, Munch]):
        super(CIGAdiv2ls, self).__init__(config)
        self.rep_out = None
    ...
    def loss_calculate(self, raw_pred: Tensor, targets: Tensor, mask: Tensor, node_norm: Tensor,
        ...
        if self.rep_out is not None:
            ...
            spu_loss=smooth_ce_loss(self.spu_out,targets,config.ood.extra_param[1],alpha=0.2,reduction='none').mean()
            #print(f'#IN# sp:{spu_loss}')
            ...
            if self.step <= -1:
                ...
                loss = cls_loss + config.ood.extra_param[0] * spu_loss
            self.mean_loss = cls_loss
            self.spec_loss = spu_loss
